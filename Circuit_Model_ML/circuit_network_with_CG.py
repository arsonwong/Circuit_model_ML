...
# if not Newton method, use conjugate gradient (for linear circuits) or gradient descent (non linear circuits)
def solve_circuit(data,diode_V_pos_delta_limit=0.5,diode_V_hard_limit=0.8,
                  convergence_RMS=1e-8,suppress_warning=False,Newton_method=True):
    cn = CircuitNetwork()
    find_ = torch.where(data.diode_nodes_tensor[:data.edge_index.shape[1] // 2,0]>=0)[0]
    diode_edges = data.diode_nodes_tensor[find_,:]
    circuit_is_linear = True
    if diode_edges.shape[0] > 0:
        circuit_is_linear = False
    ...
    for iter in range(50):
        J = -torch.autograd.functional.jacobian(lambda x_: cn(pyg.data.Data(x=x_, 
                                                                        ...
                                                                        y=data.y)), x).squeeze()
        # now J should be positive definite matrix
        J = J[rows][:, rows]
        Y = node_error[rows]
        inv_J = 1 / torch.diag(J)
        J_ = inv_J[:,None] * J
        Y_ = inv_J[:,None] * Y
        p = Y
        alpha = torch.sum((J @ p) * Y)/torch.sum((J @ p)**2)
        X = p*alpha
        # for inner_iter in range(50): #range(len(rows)+1):
        #     SS = Y.T @ Y
        #     RMS = torch.sqrt (SS / len(rows))
        #     print(RMS)
        #     if inner_iter == 0:
        #         p = Y
        #     else:
        #         beta = SS/(last_Y.T @ last_Y)
        #         p = Y + last_p*beta
        #     # alpha = SS/(p.T @ J @ p)
        #     alpha = torch.sum((J @ p) * Y)/torch.sum((J @ p)**2)
        #     X = p*alpha
...
if __name__ == "__main__":
    grid_circuit = GridCircuit(is_linear=True,has_current_source=False)
    grid_circuit.draw()
